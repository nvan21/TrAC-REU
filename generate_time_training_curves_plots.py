...
def plot_means_and_stds(means_list, stds_list, timesteps_list, series_list):
    ...
    plt.title("Pendulum Learning Comparison", fontsize=20, fontweight="bold")
    plt.xlabel(f"Simulation wall-time (min)", fontsize=14)
    ...
    plt.show()
    plt.savefig(
        "experiments/training_curves/Time Learning Curve Comparison.png", format="png"
    )
    plt.savefig(
